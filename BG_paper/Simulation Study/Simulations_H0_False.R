...
# Simulating H0 false (unpooled variance)
# oct-2018  
...
data<-read.table(file="./DATABASE_Scenarios_H0_False.csv",sep = ";", header = T, dec=",")
...
for(i in 1:dim(data)[1]){
  ...
  p1_group1 = data$p1.1[i]
  p2_group1 = data$p2.1[i]
  
  # phi_group1: Joint problability two correlated binary endpoints P(X1 =1, X2 =1)
  phi_group1 = corr*sqrt(p1_group1*(1-p1_group1)*p2_group1*(1-p2_group1)) + p1_group1*p2_group1
...
write.csv2(data,'RESULTS_H0False_CompleteUnpooled.csv2')
(t1)
